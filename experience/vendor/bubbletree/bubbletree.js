...
//var BubbleTree = function(config, onHover, onUnHover) { // TODO generator-edtapp has commented this
var BubbleTree = function(config) {
  console.log("PROCESSING BUBBLETREE 2.0.2");
  ...
  me.initPaper = function() {
    var me = this, $c = me.$container, rt = me.treeRoot,
      ...
      w = 1100, h = 550, // added by generator-edtapp
    ...
    me.height = h;
    me.paper = paper;
    console.log("me.initPaper ------> w = " + w + " h = " + h);
  ...
  me.changeView = function(token) {
    ...
    if (node !== null) {
      ...
      if (node == root || node.parent == root && node.children.length < 2) {
        ...
        rad2 = 0 - Math.max(
          hw *0.8 - tgtScale * (a2rad(node.parent.amount)+a2rad(node.amount)), // maximum visible part // used for generator-edtapp
  ...
  */
  ...
  me.urlChanged = function(hash) {
    ...
    if (!me.freshUrl) {
      ...
      // TODO edtapp-generator has commented following
      /*
      if (hash.indexOf('/~/')) {
    ...
    if (!me.freshUrl.match(me.baseUrl)) // navigating out of bubbletree page
    {
      console.log(me.freshUrl + " urlChanged returning");
      me.changeUrl();
      return;
    }
    ...
    console.log("me.urlChanged --------> me.freshUrl = " + me.freshUrl);
  };
  ...
  me.changeUrl = function() {
    ...
    // added by varsha for url correction while navigating out of bubbletree page
    if (!me.freshUrl.match(me.baseUrl)) // navigating out of bubbletree page
    {
      console.log(me.freshUrl + " is called clearing all and returning");
      $.history.clear(me.freshUrl);
      return;
    }

    console.log("me.changeUrl----------> me.freshUrl = " + me.freshUrl);
    console.log("me.changeUrl----------> me.treeRoot = " + me.treeRoot);
    
    if (me.nodesByUrlToken.hasOwnProperty(token)) {
      url = me.getUrlForNode(me.nodesByUrlToken[token]);
      console.log("me.changeUrl----------> token url" + token + "  " + url);
      console.log("me.changeUrl----------> me.freshUrl = " + me.freshUrl);
      if (me.freshUrl != url) {
        // node found but url not perfect
        console.log("me.changeUrl----------> node found but url not perfect");
        $.history.load(url);
      } else {
  ...
  me.navigateTo = function(node, fromUrlChange) {
    console.log("me.navigateTo -------> navigatine to node " + node);
